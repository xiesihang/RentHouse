        ...
        columns:[[
            ...
            {field:'operation',title:'操作',width:110,align:'center',
                formatter:function (value,row,index) {
                    return "<input type='button' value='删除' onclick='deleteOne("+row.id+")'/>" + "&nbsp;<input type='button' value='修改' onclick='dialogs(\"updateUsers\",\"修改用户\")'/>"}}]]
    });
});
//查询条件返回到后台
function search() {
    $("#dg").datagrid("load", {"name":$.trim($('#name').val()), "telephone":$.trim($('#telephone').val())})
}
...
function dialogs(id,title) {
    ...
    if(id==="updateUsers"){
        ...
        if(SelectRows.length != 1){
            ...
            $.post("selectOneUsers",{"id":row.id},function (data) {
                //将数据回显到表单中
...
function deleteBatch(SelectRows){
    $.messager.confirm("系统提示", "确定要删除<font color=red> " + SelectRows.length + " </font>条数据吗?", function(r){
        if (r){
            ...
            $.post("deleteUsersBatch",{"ids":ids},function (data) {
                if(data==1){
...
function deleteOne(id,title){
    $.messager.confirm('系统提示', '确定删除吗?', function(r){
        if (r){
            $.post("deleteUsersOne",{"id":id},function (data) {
                if(data==1){
                    $.messager.alert('系统提示',"删除成功",'info');
                    $('#dg').datagrid('reload');
                }else{
