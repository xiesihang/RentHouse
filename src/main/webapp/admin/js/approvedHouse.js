    ...
    $('#dg').datagrid({
        ...
        columns:[[
            ...
            {field:'operation',title:'操作',width:145,align:'center',
                formatter:function (value,row,index) {
                    return "<input type='button' value='取消审核' onclick='cancelApprovedHouse("+row.id+")'/>"
                }}
        ]]
    });
});

function cancelApprovedHouse(id) {
    $.post("cancelApprovedHouse",{"id":id},function (data) {
        if(data>0){
            $.messager.alert('系统提示',"取消审核成功!",'info');
            $('#dg').datagrid('reload');
        }else{
            $.messager.alert('系统提示',"取消审核失败!",'error');
            $('#dg').datagrid('reload');
        }
    },"json")
}
