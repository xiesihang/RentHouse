  ...
  <TR>
    ...
    <TD><!-- <input type="password" class="text" name="password" /> --><INPUT 
      id=user_password class=text type=password name=password> </TD></TR><!--
						...
						--></TBODY></TABLE>
<DIV class=buttons> <INPUT onclick='login()' value=登陆 type=button> <INPUT onclick='document.location="regs.jsp"' value=注册 type=button>
...
<script>
  function login() {
      ...
      $.post("/page/login",{name:name,password:password},function (data) {
          if(data==0){
              alert("用户名或密码错误")
          }else {
              alert("登录成功")
